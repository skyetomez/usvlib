...
def bandpassSOS(
    ...
    sos = butter(order, [min, max], btype="bandpass", analog=False, output="sos", fs=sr)
    _signal = deepcopy(signal)
    cleaned, *_ = sosfilt(sos, _signal)
    return cleaned
...
def drop_back(spectrograms: np.ndarray, threshold: int) -> np.ndarray:
    """
    Cleaning function replaces spectrogram points below a certain threshold.
